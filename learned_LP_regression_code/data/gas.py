...
def processRaw(rawdir, scale):
    ...
    #for i in range(len(filenames)):
        print(i)
        fpath = os.path.join(
            rawdir, "raw", "dataset_twosources_raw", sorted_fnms[i])
        pd_dataframe = pd.read_csv(fpath)
        #np_array = pd_dataframe.to_numpy()
        np_array = np.array(pd_dataframe)
        ...
        if i == 0:
            A_train, A_test = torch.empty((0, A.shape[0], A.shape[1])), torch.empty(
                (0, A.shape[0], A.shape[1]))
            B_train, B_test = torch.empty((0, B.shape[0], B.shape[1])), torch.empty(
                (0, B.shape[0], B.shape[1]))
    ...
    N = 5
    # 180
    torch.save([A_train, B_train], os.path.join(
        rawdir, "raw", "gas", "train_"+str(N) + ".dat"))
    torch.save([A_test, B_test], os.path.join(
        rawdir, "raw", "gas", "test_"+str(N) + ".dat"))
...
def getGas(raw, rawdir, scale):
    ...
    N = 5    # 180
    AB_train = torch.load(os.path.join(
        rawdir, "raw", "gas", "train_"+str(N) + ".dat"))
    AB_test = torch.load(os.path.join(
        rawdir, "raw", "gas", "test_"+str(N) + ".dat"))
